...
EXPORT_SYMBOL(jzfb_vsync_thread);
...
int vsync_soft_set(int data)
{
	...
	vsync_delay = data;
	vsync_data.vsync_thread = kthread_run(jzfb_vsync_thread, &vsync_delay, "jzfb-vsync");
	if (vsync_data.vsync_thread == ERR_PTR(-ENOMEM)) {
		return -1;
	}
	return 0;
}
EXPORT_SYMBOL(vsync_soft_set);

void vsync_soft_stop()
{
	kthread_stop(vsync_data.vsync_thread);
}
EXPORT_SYMBOL(vsync_soft_stop);
...
int vsync_wait(struct fb_info *info,unsigned long arg)
{
	...
	ret = copy_to_user(argp, vsync_data.timestamp.value + vsync_data.timestamp.rp, sizeof(u64));
	vsync_data.timestamp.rp = (vsync_data.timestamp.rp + 1) % TIMESTAMP_CAP;
	return ret;
}
EXPORT_SYMBOL(vsync_wait);
