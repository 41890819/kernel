...
static void jzfb_videomode_to_var(struct fb_var_screeninfo *var, const struct fb_videomode *mode)
{
	var->xres = mode->xres;
	var->yres = mode->yres;
...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	jzfb->base = ioremap(LCDC_IOBASE, 0x1800);
	if (reg_read(jzfb, LCDC_CTRL) & LCDC_CTRL_ENA) {
		int count = 20000;
		unsigned long ctrl = reg_read(jzfb, LCDC_CTRL);
		ctrl |= LCDC_CTRL_DIS;
