...
static void jzfb_config_fg0(struct fb_info *info)
{
	...
	if (jzfb->fmt_order == FORMAT_X8B8G8R8) {
		rgb_ctrl = LCDC_RGBC_RGBFMT | LCDC_RGBC_ODD_BGR | LCDC_RGBC_EVEN_BGR;
	} else {
		/* default: FORMAT_X8R8G8B8 */
		rgb_ctrl = LCDC_RGBC_RGBFMT | LCDC_RGBC_ODD_RGB | LCDC_RGBC_EVEN_RGB;
	}
...
static int jzfb_calculate_size(struct fb_info *info, struct jzfb_display_size *size)
{
	struct jzfb *jzfb = info->par;
	struct fb_videomode *mode = info->mode;
	...
	size->panel_line_size = ALIGN(mode->xres, PIXEL_ALIGN) * (jzfb->osd.fg0.bpp >> 3);
	/* word aligned and in word */
...
jzfb_config_fg1_dma(struct fb_info *info, struct jzfb_display_size *size)
{
	...
	jzfb->fg1_framedesc->next = jzfb->framedesc_phys +
	...
	jzfb->fg1_framedesc->cpos = LCDC_CPOS_BPP_18_24 | jzfb->osd.fg0.y << LCDC_CPOS_YPOS_BIT
		| jzfb->osd.fg0.x | LCDC_CPOS_PREMULTI | LCDC_CPOS_COEF_SLE_3;

	jzfb->fg1_framedesc->desc_size = size->height_width | 0xff << LCDC_DESSIZE_ALPHA_BIT;
...
static int jzfb_prepare_dma_desc(struct fb_info *info)
{
	...
	framedesc[0] = kzalloc(sizeof(struct jzfb_framedesc) * (jzfb->desc_num - 1), GFP_KERNEL);
...
static void jzfb_enable(struct fb_info *info)
{
	...
	if (jzfb->is_lcd_en == 0) {
		...
		/* set lcd dither enable RGB888 -> RGB666 */
		ctrl = reg_read(jzfb, LCDC_ENH_CFG);
		ctrl |= LCDC_ENH_CFG_ENH_EN;
		ctrl |= LCDC_ENH_CFG_DITHER_EN;
		reg_write(jzfb, LCDC_ENH_CFG, ctrl);

		ctrl = LCDC_ENH_DITHERCFG_DITHERMD_RED_MASK |
			LCDC_ENH_DITHERCFG_DITHERMD_GREEN_MASK |
			LCDC_ENH_DITHERCFG_DITHERMD_BLUE_MASK;
		reg_write(jzfb, LCDC_ENH_DITHERCFG, ctrl);
		...
		if (jzfb->pdata->lcd_type != LCD_TYPE_SLCD) {
			if (reg_read(jzfb, LCDC_CTRL) & LCDC_CTRL_ENA) {
				...
				if (count >= 0) {
					...
					dev_err(info->dev, "LCDC normal disable state wrong\n");
				}
			}
		} else {
...
static int jzfb_alloc_devmem(struct jzfb *jzfb)
{
	...
	jzfb->framedesc[0] = dma_alloc_coherent(jzfb->dev,
	 ...
	 */
	jzfb->vidmem = dma_alloc_coherent(jzfb->dev, jzfb->vidmem_size,
...
{
	dma_free_coherent(jzfb->dev, jzfb->vidmem_size, jzfb->vidmem, jzfb->vidmem_phys);
...
static inline int timeval_sub_to_us(struct timeval lhs, struct timeval rhs)
{
	int sec, usec;
...
static int jzfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
		...
		if(showFPS) {
			...
			default:
				if (showFPS > 3) {
					...
					do_gettimeofday(&time_now);
					interval_in_us = timeval_sub_to_us(time_now, time_last);
					if (interval_in_us > USEC_PER_SEC * showFPS ) {
						d = fpsCount / showFPS;
	...
	if (jzfb->is_buffer_used)
	        wait_for_completion(&jzfb->user_completion);
	...
	if (jzfb->pdata->lcd_type != LCD_TYPE_INTERLACED_TV &&
		...
		jzfb->framedesc[0]->databuf = jzfb->vidmem_phys	+ jzfb->frm_size * next_frm;
		if (!jzfb->is_lcd_en)
			return -EINVAL;;
...
static void jzfb_set_alpha(struct fb_info *info, struct jzfb_fg_alpha *fg_alpha)
{
	...
	if (fg_alpha->enable) {
		...
		for (i = 0; i < desc_num; i++) {
			...
			(framedesc + i)->desc_size |= ((fg_alpha->value << LCDC_DESSIZE_ALPHA_BIT)
...
int jzfb_set_fg0_alpha(int is_pixel_alpha, unsigned int alpha)
{
	struct fb_info *info;
	...
	if (jzfb_get_controller_bpp(jzfb) == 16) {
		printk("RGB565 not support alpha mode\n");
		return 0;
	}

	printk("[ jzfb_set_fg0_alpha ] is_pixel_alpha = %d, alpha = %d\n", is_pixel_alpha, alpha);

	info = jzfb->fb;

	fg_alpha.fg = 0;
	fg_alpha.enable = 1;
	fg_alpha.mode = is_pixel_alpha;
	fg_alpha.value = alpha;
	jzfb_set_alpha(info, &fg_alpha);

	return 1;
}
EXPORT_SYMBOL(jzfb_set_fg0_alpha);
...
static void jzfb_set_background(struct fb_info *info, struct jzfb_bg *background)
{
	...
	bg_value |= (background->green << LCDC_BGC_GREEN_OFFSET & LCDC_BGC_GREEN_MASK);
	bg_value |= (background->blue << LCDC_BGC_BLUE_OFFSET & LCDC_BGC_BLUE_MASK);
...
static void jzfb_set_colorkey(struct fb_info *info, struct jzfb_color_key *color_key)
{
	...
	tmp |= (color_key->green << LCDC_KEY_GREEN_OFFSET & LCDC_KEY_GREEN_MASK);
...
static int jzfb_set_foreground_position(struct fb_info *info, struct jzfb_fg_pos *fg_pos)
{
	int i;
	int desc_num;
	struct jzfb *jzfb = info->par;
...
static int jzfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case JZFB_SET_VIDMEM:
		if (copy_from_user(&jzfb->vidmem_phys, argp, sizeof(unsigned int)))
			return -EFAULT;
		break;
	case JZFB_SET_MODE:
	...
	case JZFB_SET_FG_SIZE:
		if (copy_from_user(&osd.fg_size, argp, sizeof(struct jzfb_fg_size))) {
	...
	case JZFB_GET_FG_SIZE:
		...
		osd.fg_size.h = (value & LCDC_SIZE_HEIGHT_MASK) >> LCDC_SIZE_HEIGHT_BIT;
		if (copy_to_user(argp, &osd.fg_size, sizeof(struct jzfb_fg_size))) {
			dev_info(info->dev, "copy FG size to user failed\n");
			return -EFAULT;
		}
		break;
	case JZFB_SET_FG_POS:
		if (copy_from_user(&osd.fg_pos, argp, sizeof(struct jzfb_fg_pos))) {
	...
	case JZFB_GET_FG_POS:
		...
		osd.fg_pos.y = (value & LCDC_XYP_YPOS_MASK) >> LCDC_XYP_YPOS_BIT;
	...
	case JZFB_GET_BUFFER:
	        if (copy_from_user(&value, argp, sizeof(unsigned int)))
		        return -EFAULT;
		if (value == LOCK_BUFFER) {
			mutex_lock(&jzfb->lock);
			jzfb->is_buffer_used = 1;
			jzfb->is_buffer_suspend = 0;
			mutex_unlock(&jzfb->lock);
		...
		} else if (value == UNLOCK_BUFFER) {
			mutex_lock(&jzfb->lock);
			if (jzfb->is_buffer_suspend) {
			        complete(&jzfb->user_completion);
			}
			jzfb->is_buffer_suspend = 0;
			jzfb->is_buffer_used = 0;
			mutex_unlock(&jzfb->lock);
		...
		break;
	case JZFB_SET_ALPHA:
		if (copy_from_user(&osd.fg_alpha, argp, sizeof(struct jzfb_fg_alpha))) {
	...
	case JZFB_SET_BACKGROUND:
		if (copy_from_user(&osd.background, argp, sizeof(struct jzfb_bg))) {
	...
	case JZFB_SET_COLORKEY:
		if (copy_from_user(&osd.color_key, argp, sizeof(struct jzfb_color_key))) {
			dev_info(info->dev, "copy colorkey from user failed\n");
			return -EFAULT;
		}
...
static int jzfb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	pgprot_val(vma->vm_page_prot) |= _CACHE_CACHABLE_NO_WA;// Write-Acceleration
...
static void jzfb_early_suspend(struct early_suspend *h)
{
	...
	clk_disable(jzfb->pclk); // Pixel Clock
	clk_disable(jzfb->clk); // LCDC Clock
	clk_disable(jzfb->pwcl); // LCDC Power
	mutex_unlock(&jzfb->lock);
...
static void jzfb_change_dma_desc(struct fb_info *info)
{
	...
	if (mode->pixclock) {
		...
		clk_set_rate(jzfb->pclk, rate);
		clk_enable(jzfb->pclk);
		dev_dbg(jzfb->dev, "LCDC PixClock except %lu set %lu\n",
...
static void jzfb_display_v_color_bar(struct fb_info *info)
{
	...
	dev_info(info->dev, "LCD V COLOR BAR w,h,bpp(%d,%d,%d) jzfb->vidmem=%p\n",
		 w, h, bpp, jzfb->vidmem);
...
static void jzfb_display_h_color_bar(struct fb_info *info)
{
	...
	dev_info(info->dev, "LCD H COLOR BAR w,h,bpp(%d,%d,%d), jzfb->vidmem=%p\n",
		 w, h, bpp, jzfb->vidmem);
...
static ssize_t vsync_skip_r(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct jzfb *jzfb = dev_get_drvdata(dev);
...
static int vsync_skip_set(struct jzfb *jzfb, int vsync_skip)
{
	...
	printk("vsync_skip_ratio = %d, vsync_skip_map = 0x%08x\n",
	       jzfb->vsync_skip_ratio, jzfb->vsync_skip_map);
	mutex_unlock(&jzfb->lock);
...
{
	printk("Please 'echo NUM > show_fps' to select an available method\n");
			 ...
			 const char *buf, size_t n)
{
	...
	if (num > 3) {
		printk("Please 'cat show_fps' to view available methods\n");
	} else {
		showFPS = num;
		if (showFPS == 3)
...
static ssize_t lcd_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%u", jzfb->pdata->lcd_type);
}

static ssize_t lcd_test_pattern(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct jzfb *jzfb = dev_get_drvdata(dev);
...
static DEVICE_ATTR(show_lcd_type, S_IRUGO|S_IWUSR, lcd_type_show, NULL);
static DEVICE_ATTR(test_pattern, S_IRUGO|S_IWUSR, lcd_test_pattern, NULL);
...
static int __devinit jzfb_probe(struct platform_device *pdev)
{
	...
	init_completion(&jzfb->user_completion);
 ...
 err_put_clk:
	...
	if (jzfb->pwcl) clk_put(jzfb->pwcl);
...
static void jzfb_shutdown(struct platform_device *pdev)
{
	...
	mutex_lock(&jzfb->suspend_lock);
	if (jzfb->is_suspend != 1) fb_blank(jzfb->fb, FB_BLANK_POWERDOWN);
	jzfb->is_suspend = 1;
	mutex_unlock(&jzfb->suspend_lock);
};
