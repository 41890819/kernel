...
#ifdef CONFIG_BOARD_PARAMOUNT
	if (brightness == 0) {
	    tmp_brightness = pb->cur_brightness;
	    for(; tmp_brightness > 0; tmp_brightness -= 10){
		pwm_disable(pb->pwm);
		tmp_value = pb->lth_brightness +
			(tmp_brightness * (pb->period - pb->lth_brightness) / max);
		pwm_config(pb->pwm, tmp_value, pb->period);
		pwm_enable(pb->pwm);
		mdelay(20);
	    }
		pb->cur_brightness = 0;
...
{
	struct pwm_bl_data *pb = container_of(rnb, struct pwm_bl_data, nb); 

	mutex_lock(&pb->pwm_lock);
	if (pb->notify)
		pwm_config(pb->pwm, pb->notify(pb->dev, 0), pb->period);
	else
		pwm_config(pb->pwm, 0, pb->period);
	pwm_disable(pb->pwm);
	mutex_unlock(&pb->pwm_lock);
...
{
	struct pwm_bl_data *pb = container_of(h, struct pwm_bl_data, bk_early_suspend);
