...
// Control
#define RESET(n)     ecx336af_pdata->gpio_reset != -1 ? gpio_direction_output(ecx336af_pdata->gpio_reset, n) : 0

// SPI
#define CS(n)     ecx336af_pdata->gpio_spi_cs != -1 ? gpio_direction_output(ecx336af_pdata->gpio_spi_cs, n) : 0
#define SCK(n)    ecx336af_pdata->gpio_spi_clk != -1 ? gpio_direction_output(ecx336af_pdata->gpio_spi_clk, n) : 0
#define SDO(n)    ecx336af_pdata->gpio_spi_sdi != -1 ? gpio_direction_output(ecx336af_pdata->gpio_spi_sdi, n) : 0
#define SDI()     ecx336af_pdata->gpio_spi_sdo != -1 ? gpio_get_value(ecx336af_pdata->gpio_spi_sdo) : 0
 ...
 */
static void SPI_WRITE_REG(unsigned char addr, unsigned char *value, int len)
{
	int i = 0;
	int byte = 0;
	...
	{
		int i = 0;
		printk("Write %02x reg value is", addr);
		for (i = 0; i < len; i++)
			printk(" %02x", value[i]);
		printk("\n");
	}
...
static void SPI_GET_VALUE(unsigned char *value, int len)
{
	int i = 0;
	int byte = 0;
	unsigned char temp = 0;
 ...
 */
static void SPI_READ_REG(unsigned char addr, unsigned char *value, int len)
{
	unsigned char RD_ON;
	RD_ON = 0x01;
	...
	{
		int i = 0;
		printk("Read %02x reg value is", addr);
		for (i = 0; i < len; i++)
			printk(" %02x", value[i]);
		printk("\n");
	}
...
static void ecx336af_write_reg(unsigned char addr, unsigned char value)
{
	SPI_WRITE_REG(addr, &value, 1);
}

static unsigned char ecx336af_read_reg(unsigned char addr)
{
 ...
 */
static void ECX336AF_PS_ON(void)
{
	unsigned char ps0;
...
static void ECX336AF_PS_OFF(void)
{
	unsigned char ps0;
	SPI_READ_REG(0x00, &ps0, 1);
	ps0 &= (~0x1);
...
void ECX336AF_POWER_ON(struct platform_ecx336af_data *pdata)
{
        if (ecx336af_pdata == NULL)
	  ecx336af_pdata = pdata;

	if (ecx336af_pdata->power_1v8 && regulator_is_enabled(ecx336af_pdata->power_1v8))
...
void ECX336AF_POWER_OFF(struct platform_ecx336af_data *pdata)
{
        if (ecx336af_pdata == NULL)
	  ecx336af_pdata = pdata;
...
void Initial_IC(struct platform_ecx336af_data *pdata)
{
	//printk("Initial_IC start now\n");
        if (ecx336af_pdata == NULL)
	  ecx336af_pdata = pdata;
	...
	//ecx336af_pdata = NULL;
	//printk("Initial_IC stop now\n");
}

void ECX336AF_BACKLIGHT(unsigned char brightness){
  ecx336af_write_reg(0x2A, brightness);
}
