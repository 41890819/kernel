...
static int himax7097_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev->lcd = lcd_device_register("himax7097", &client->dev, dev, &himax7097_ops);
	if (IS_ERR(dev->lcd)) {
		ret = PTR_ERR(dev->lcd);
		dev->lcd = NULL;
...
static struct i2c_driver himax7097_driver = {
	.driver		= {               
		.name	= "himax7097",
		.owner	= THIS_MODULE,
	},
...
static int __init himax7097_lcd_init(void)
{
	return i2c_add_driver(&himax7097_driver);
}

static void __exit himax7097_lcd_exit(void)
{
	i2c_del_driver(&himax7097_driver);
}

module_init(himax7097_lcd_init);
module_exit(himax7097_lcd_exit);
