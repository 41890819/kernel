...
#ifdef CONFIG_GLASS_PDMA
__initdata static int firmware[] = {
	#include "firmware_glass.hex"
};
...
#if defined(CONFIG_NAND) || defined(CONFIG_GLASS_PDMA)
...
#ifdef CONFIG_GLASS_PDMA
	for(i = 0;i < 6; i++)
		mailbox_pend_addr->gpio[i] = 0;
	for(i = 0;i < 4; i++)
		mailbox_pend_addr->uart[i] = 0;
#endif
	return 0;
}
...
irqreturn_t pdma_int_handler(int irq_pdmam, void *dev)
{
	...
	if(pending & DMINT_N_IP){
#ifdef CONFIG_SERIAL_JZ47XX_UART0_PDMA
		void pdma_handle_irq(int id, unsigned int status);

		{
			if (mailbox_pend_addr->uart[0]) {
				pdma_handle_irq(0, mailbox_pend_addr->uart[0]);
