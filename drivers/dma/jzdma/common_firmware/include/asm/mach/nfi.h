 ...
 *
 * JZ47805 nfi register definition.
 *
...
/* NFI enable Register */
#define NAND_NFCR_CSMOD(n)		(0x1 << (15 +(n) - 1))	 /* when the nand is common and NAND_NFCR_CSEN is 1, the CSNOD(n)
									should be set 1. */
#define NAND_NFCR_CSEN			(0x1 << 15)		 /* if the bit is 1, we should be select chip with NFCS when the nand
...
#define __pn_enable()           (REG_NAND_PNCR |= NAND_PNCR_PNRST | NAND_PNCR_PNEN)
#define __pn_disable()          (REG_NAND_PNCR &= ~(NAND_PNCR_PNEN))

#define __nand_counter0_enable()	\
do{									\
	unsigned int tmp = REG_NAND_PNCR;				\
...
#define __nand_counter_disable()	(REG_NAND_PNCR &= ~(NAND_PNCR_BITEN))
#define	__nand_read_counter(n)		((n) = REG_NAND_BCNT)
