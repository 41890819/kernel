...
static void transmit_chars_to_pdma(struct uart_jz47xx_port *up)
{
	...
	if (up->port.x_char) {
		...
		waddr = (waddr + 1) & (TCSM_UART_BUF_LEN - 1);
		count++;
		up->port.icount.tx++;
		up->port.x_char = 0;//transmit finish x_char=0
	...
	if (((count == 0) && uart_circ_empty(xmit)) || uart_tx_stopped(&up->port)) {//xmit is empty or stop tx
		serial_jz47xx_stop_tx(&up->port);
		return;
	}

	while (count < TCSM_UART_BUF_LEN) {
...
static int serial_jz47xx_suspend(struct device *dev)
{
	...
	if (ttyport->flags & ASYNC_CTS_FLOW) {
		...
		if (serial_jz47xx_get_mctrl(&up->port) & TIOCM_CTS){
			up->suspend_cts = 1;
		} else {
			up->suspend_cts = 0;
		}
