...
static int jzdmic_open(struct inode *inode, struct file *filp)
{
	...
	if(jzdmic->dev_opened) {
		printk("dmic recorder is busy!\n");
		return -EBUSY;	
	}
	jzdmic->dev_opened = 1;
	jzdmic->clearnoise_enabled = 1;
	jzdmic->clearnoise_cnt = 1920;
...
static int jzdmic_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	while( mcount > 0) {
		...
		while(1) {
			...
			if(jzdmic->clearnoise_enabled == 1){
			  //printk("record memset\n");
				
			  if (jzdmic->clearnoise_cnt >= nread){
			    jzdmic->clearnoise_cnt-=nread;
			    memset(buf, 0 , nread);
			  }else{
			    memset(buf, 0 , jzdmic->clearnoise_cnt);
			    jzdmic->clearnoise_enabled = 0;
			    jzdmic->clearnoise_cnt = 1920;
			  }
			}
