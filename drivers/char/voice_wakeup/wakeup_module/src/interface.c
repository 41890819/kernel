...
static inline void powerdown_wait(int force)
{
	unsigned int opcr;
	unsigned int cpu_no;
	unsigned int lcr;
	...
	/* if(force){ */
	/* 	while((REG_DMIC_FSR & 0x7f) > (DMIC_FIFO_THR - 20)) { */
	/* 		REG_DMIC_DR; */
	/* 	} */
	/* } */
...
static void dmic_cpu_relax(int force)
{
	unsigned int val;
	if(force) {
		flush_dcache_all();
		__asm__ __volatile__("sync");		
		powerdown_wait(force);
	} /* else { */
	/* 	val = REG_DMIC_FCR; */
	/* 	val &= ~0x7f; */
	/* 	val |= 128 / 4; */
