...
static int rtc_save(void)
{
	...
	if((jzrtc_readl(RTC_RTCSAR) + 1) < jzrtc_readl(RTC_RTCSR)) {
...
{
	unsigned int rtcsr = jzrtc_readl(RTC_RTCSR);

	//if(((jzrtc_readl(RTC_RTCSR)+ALARM_VALUE) >= old_config.alarm_val) && (old_config.alarm_val != 0)) {
	if(((rtcsr + ALARM_VALUE) >= old_config.alarm_val) && (old_config.systimer_configed == 1)) {
		/* kernel alarm arrived, priority higher. wakeup OS, imediatly */
		if(rtcsr + 1 >= old_config.alarm_val) {
			return SYS_TIMER;
		}

		rtc_set_alarm(old_config.alarm_val - rtcsr);
