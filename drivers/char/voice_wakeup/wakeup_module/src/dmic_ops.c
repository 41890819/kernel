...
{
	return ((REG_DMIC_FSR & 0x7f) < (DMIC_FIFO_THR - 30)) && (last_dma_count == REG_DMADTC(5)) ? 1 : 0;
}

// ENABLE_LP_MODE : In LowerPowerMode, DMIC_CLK output 1.2MHz, otherwise DMIC_CLK output 2.4MHz
// ENABLE_SW_LR : Enable SW_LR will exchange left and right channel output
...
int dmic_init_mode(int mode)
{
	...
	switch (mode) {
		...
		case DEEP_SLEEP:
			...
			//REG_DMIC_CR0 &= ~(1<<8 | 1 << 12);
#if defined(ENABLE_LP_MODE)
			REG_DMIC_CR0 |= (1 << 3);
#else
			REG_DMIC_CR0 &= ~(1 << 3);
#endif

#if defined(ENABLE_SW_LR)
			REG_DMIC_CR0 |= (1 << 11);
#else
			REG_DMIC_CR0 &= ~(1 << 11);
#endif
