...
static unsigned int cal_del_time(struct timeval now, struct timeval last)
{
	int sec, usec;
...
static unsigned int del_time = 0;
#endif
...
#ifdef CONFIG_VIDEO_JZ_HS_ISP
	if((button->code == KEY_CAMERA) && (getCameraKey_WakeUp()))
		    return;	       
#endif
#if defined(CONFIG_BOARD_CRUISE)
	if (button->gpio == GPIO_PD(18)) { // KEY_A
		do_gettimeofday(&time_now);
		del_time = cal_del_time(time_now, time_last);

		if (last_state == 0) {
			if (del_time < 300 * 1000)
				return ;
			if (state == 0)
				return ;
		}
