...
#include "IT7236_JZ_7.0.A.6_20150615.h"
...
static void updater_touch(struct ite7236_tp_data *ite7236_tp, int x , int y, int z)
{
...
static void ite7236_work_handler(struct work_struct *work){
	...
	i2cReadFromIt7280(tp->client, 0x00, pucBuffer, 2);
	i2cReadFromIt7280(tp->client, 0x07, Buffer, 1);
	
	// printk("Raw : %4d 0x%02x\n", pucBuffer[1], Buffer[0]);
	
	if (Buffer[0] == 0x40 && pucBuffer[1] == 0 && tp->status != ITE_STATUS_TOUCH) { // touch back area
		tp->status = ITE_STATUS_BACK;
	} else if (Buffer[0] != 0x40 && Buffer[0] != 0 && pucBuffer[1] != 0x00) { // touch move area
		if (tp->last_x == 0)
			updater_touch(tp, 0, 0, 0x81); // press event
		else {
			delta = pucBuffer[1] - tp->last_x;
			// Limit distance between two point, which will avoid flip
			if (delta > 7) {
				delta = 7;
			} else if (delta < -7) {
				delta = -7;
			...
			updater_touch(tp, delta * SCALE, 0, 0x81); // move event
		}
		tp->last_x = pucBuffer[1];
		tp->status = ITE_STATUS_TOUCH;
	} else if (Buffer[0] == 0 && pucBuffer[1] == 0x00) { // touch end
		if (tp->status == ITE_STATUS_FREE) {
			return;
		} else if (tp->status == ITE_STATUS_BACK) {
			updater_touch(tp, 4, 0, 0x11); // back event
		} else if (tp->status == ITE_STATUS_TOUCH) {
			updater_touch(tp, 0, 0, 0x2); // end event
			tp->last_x = 0;
		}
		tp->status = ITE_STATUS_FREE;
	}
}
