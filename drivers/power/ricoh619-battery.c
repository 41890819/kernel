 ...
 *
 */
#define RICOH61x_BATTERY_VERSION "RICOH61x_BATTERY_VERSION: 2015.3.31 V3.1.1.3"
...
#endif
	ret = ricoh61x_read(info->dev->parent, POWEROFF_HISTORY_REG, &reg_val);
	if (ret < 0) {
		dev_err(info->dev,"PMU:Error in reading POWEROFF_HISTORY_REG %d\n", ret);
		return ret;
	}
	printk("power off history:%d\n",reg_val);
	ret = ricoh61x_read(info->dev->parent, POWERON_HISTORY_REG, &reg_val);
	if (ret < 0) {
...
static __devinit int ricoh61x_battery_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&info->suspend_lock, WAKE_LOCK_SUSPEND, "ricoh61x-charger");
	...
	val &= 0x7f;
	pr_info("PMU:ricoh61x_battery_probe : pswr = %d\n", val);
	...
	*/
	...
	/* Set reset time to 10s */
	{
		int error = 0;
		int value = 0;
		error = ricoh61x_read(info->dev->parent, RICOH61x_PWR_ON_TIMSET, &value);
		if (error < 0)
			dev_err(info->dev, "Read RICOH61x_PWR_ON_TIMSET error\n");
		else
			dev_err(info->dev, "Read RICOH61x_PWR_ON_TIMSET value %d\n", value);
		value &= 0x0F;
		value |= 0x60;
		error = ricoh61x_write(info->dev->parent, RICOH61x_PWR_ON_TIMSET, value);
		dev_err(info->dev, "Read RICOH61x_PWR_ON_TIMSET value %d\n", value);
		if (error < 0)
			dev_err(info->dev, "Write RICOH61x_PWR_ON_TIMSET error\n");
	}
	pr_info("PMU:ricoh61x_battery_probe success! \n");
...
static int __devexit ricoh61x_battery_remove(struct platform_device *pdev)
{
	...
	val &= 0x7f;
	pr_info("PMU:ricoh61x_battery_remove : pswr = %d\n", val);
...
static int power_off(struct platform_device *pdev) { 
        ...
        err = ricoh61x_set_bits(pdev->dev.parent, CC_CTRL_REG, 0x01); 
        if(err) 
                printk("Error when clean the FG CC_SUM reg!!\n"); 

        err = ricoh61x_bulk_writes(pdev->dev.parent, 
                        CC_SUMREG3_REG, 4, cc_sum_reg); 

        if(err) 
                printk("Error when clean the FG CC_SUM reg!!\n"); 

        err = ricoh61x_clr_bits(pdev->dev.parent, CC_CTRL_REG, 0x01); 
...
static int ricoh61x_init_charge_interrupt(struct ricoh61x_battery_info *info)
{
	int err;
	uint8_t val;
...
static int ricoh61x_battery_resume(struct device *dev)
{
	...
	//ricoh61x_init_charge_interrupt(info);
	return 0;
}
