...
static ssize_t acm_serial_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct android_usb_function *f = dev_get_drvdata(dev);
	struct acm_function_config *config = f->config;
...
static ssize_t acm_serial_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct android_usb_function *f = dev_get_drvdata(dev);
	struct acm_function_config *config = f->config;
	...
	sscanf(buf, "%d", &value);
	if (value >= 0 || value < MAX_ACM_INSTANCES)
		register_acm_serial(value);
	else
		unregister_acm_serial();
	config->serialno = value;
	return size;
}

static DEVICE_ATTR(serial, S_IRUGO | S_IWUSR, acm_serial_show, acm_serial_store);

static struct device_attribute *acm_function_attributes[] = { &dev_attr_instances,  &dev_attr_serial, NULL };
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	int			gcnum, id, ret;
#if defined (CONFIG_JZ4780_EFUSE) || defined (CONFIG_JZ4775_EFUSE) || defined(CONFIG_JZ_EFUSE_V12)
	uint32_t		chip_id[4];
#endif
	...
	snprintf(serial_string, sizeof(serial_string) - 1,
		 "%08X%08X", (chip_id[0] + chip_id[1]), (chip_id[2] + chip_id[3]));
#else
