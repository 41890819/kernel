...
#if defined(CONFIG_VIDEO_OV5640) && defined(CONFIG_BOARD_MONDAY)
static struct regulator * pmu_dvdd;
#endif
...
static int ovisp_subdev_sensor_init(struct ovisp_camera_dev *camdev, 
				    ...
				    int index)
{
	...
	struct v4l2_subdev *sd = csd->sd;
	struct sensor_info *si = container_of(sd, struct sensor_info, sd);
	struct isp_i2c_grp_w grp;
	struct i2c_board_info *board_info = client->board_info;
	int ret;
	...
	if (client->flags & CAMERA_CLIENT_POWER_SUPPLY_ALWAYS) {
		cfmt->vmfmt.width = 0;
		cfmt->vmfmt.height = 0;
		memset(&cfmt->fmt_data, 0, sizeof(cfmt->fmt_data));

		grp.si = si;
		grp.dev_i2c_id = board_info->addr;
		grp.i2c_flag = 0;

		ret = v4l2_subdev_call(csd->sd, video, try_mbus_fmt, &cfmt->vmfmt);
		if (ret && ret != -ENOIOCTLCMD) {
			ISP_PRINT(ISP_ERROR,"Failed to try device format at system startup\n");
			return -EINVAL;
		}
		ret = v4l2_subdev_call(csd->sd, video, g_mbus_fmt, &cfmt->vmfmt);
		if (ret && ret != -ENOIOCTLCMD) {
			ISP_PRINT(ISP_ERROR,"Failed to get device format of preview\n");
			return -EINVAL;
		}

		if (si->write_mode == ISP_GROUP_WRITE) {
			/* ISP_GROUP_WRITE mode */
			ret = isp_dev_call(camdev->isp, set_camera_fmt, &cfmt->vmfmt, &grp);
			if (ret && ret != -ENOIOCTLCMD) {
				ISP_PRINT(ISP_ERROR,"Failed to set device format of preview\n");
				return -EINVAL;
			}
		} else if (si->write_mode == I2C_WRITE) {
			/* I2C_WRITE mode */
					...
					int index)
{
...
#if defined(CONFIG_VIDEO_OV5640) && defined(CONFIG_BOARD_MONDAY)
	if(!regulator_is_enabled(pmu_dvdd))
		regulator_enable(pmu_dvdd);
#endif
	...
	if(client->flags & CAMERA_CLIENT_VCM){
	        ret = client->vcm_client->power(1);
		if (ret)
			goto err;
	}
	...
	if (camdev->input >= 0) {
		...
		if ( !(client->flags & CAMERA_CLIENT_POWER_SUPPLY_ALWAYS) ) {
			ret = ovisp_subdev_sensor_init(camdev, client, index);
			if (ret)
				goto err;
		}
		ISP_PRINT(ISP_INFO,"--%s:%d\n", __func__, __LINE__);
...
#if defined(CONFIG_VIDEO_OV5640) && defined(CONFIG_BOARD_MONDAY)
	if(regulator_is_enabled(pmu_dvdd))
		regulator_disable(pmu_dvdd);
#endif

	if(client->flags & CAMERA_CLIENT_VCM){
	        ret = client->vcm_client->power(0);
		if (ret)
			return -EINVAL;
	}
					...
					int index)
{
	...
	if(client->flags & CAMERA_CLIENT_VCM){
		csd->sensor_af_flag = true;
	        csd->vcm_sd = v4l2_i2c_new_subdev_board(&camdev->v4l2_dev,
							csd->i2c_adap,
							client->vcm_client->board_info,
							NULL);
		if (csd->vcm_sd) {
			ISP_PRINT(ISP_INFO,"csd->sd->name = %s\n",csd->sd->name);
		}
		if (!csd->vcm_sd) {
			ISP_PRINT(ISP_ERROR,"Cannot get subdev(%s)\n", client->vcm_client->board_info->type);
			clk_put(csd->mclk);
			i2c_put_adapter(csd->i2c_adap);
			err = -EIO;
			goto subdev_power_off;
		...
		csd->sensor_af_flag = false;
	}
	if (client->flags & CAMERA_CLIENT_POWER_SUPPLY_ALWAYS) {
		ret = ovisp_subdev_sensor_init(camdev, client, index);
		if (ret) {
			ISP_PRINT(ISP_ERROR,"Failed to init sensor subdev(%s)\n",
				  client->board_info->type);
			clk_put(csd->mclk);
			clk_put(csd->mclk);
			i2c_put_adapter(csd->i2c_adap);
...
static int ovisp_camera_start_capture(struct ovisp_camera_dev *camdev)
{
	...
	cap.si = container_of(csd->sd, struct sensor_info, sd);
	capture->running = 1;
	ret = isp_dev_call(camdev->isp, start_capture, &cap);
	if (ret < 0 && ret != -ENOIOCTLCMD) {
		ISP_PRINT(ISP_WARNING, "Start capture failed %d\n", ret);
		ret = v4l2_subdev_call(csd->sd, core, soft_reset, 0x02);
		if (ret < 0) {
			ISP_PRINT(ISP_ERROR, "reset sensor failed %d\n", ret);
			return -EINVAL;
		} else {
			ret = isp_dev_call(camdev->isp, start_capture, &cap);
			if (ret < 0) {
				ISP_PRINT(ISP_ERROR, "start_capture failed again %d\n", ret);
				return -EINVAL;
			}
		}
	}
...
static int ovisp_camera_start_streaming(struct ovisp_camera_dev *camdev)
{
	...
	struct isp_i2c_grp_w grp;
	struct sensor_info *si = container_of(sd, struct sensor_info, sd);
	struct i2c_board_info *board_info = csd->client->board_info;
	int ret;
	...
	ISP_PRINT(ISP_INFO,"Set format(%s).\n", camdev->snapshot ? "snapshot" : "preview");
	...
	if (ret < 0 && ret != -ENOIOCTLCMD) {
		ISP_PRINT(ISP_ERROR,"Failed to set isp format of pre_fmt\n");
		return -EINVAL;
	}

	/* ret = isp_dev_call(camdev->isp, get_camera_info, &cfmt->fmt_data); */
	/* if (ret && ret != -ENOIOCTLCMD) { */
	/* 	ISP_PRINT(ISP_ERROR,"Failed to get device format of take photo\n"); */
	/* 	return -EINVAL; */
	/* } */

	if(camdev->first_init) {
		if(si->write_mode != ISP_CMD_GROUP_WRITE)
			ret = v4l2_subdev_call(csd->sd, video, s_stream, 1);
		camdev->first_init = 0;
	}

	if (si->support_group_lunch == true) {
		ret = isp_dev_call(camdev->isp, set_camera_grouphold_mode, &grp);
	}
...
static int ovisp_camera_irq_notify(unsigned int status, void *data)
{
	...
	if (status & ISP_NOTIFY_DROP_FRAME){
		...
		if(status & ISP_NOTIFY_DROP_FRAME1){
			capture->drop_frames++;
			if(capture->active[1] == NULL){
				spin_lock_irqsave(&camdev->slock, flags);
				if (!list_empty(&capture->list)){
					buf = list_entry(capture->list.next,
						struct ovisp_camera_buffer, list);
...
static int ovisp_vb2_start_streaming(struct vb2_queue *vq)
{
	...
	ret = ovisp_camera_start_streaming(camdev);
	if (ret < 0) {
		return ret;
	}	
	...
	if (!capture->running) {
		...
		ret = ovisp_camera_start_capture(camdev);
	}
	return ret;
}
...
static int ovisp_vidioc_s_tlb_base(struct file *file, void *priv, unsigned int *tlb_base)
{
	struct ovisp_camera_dev *camdev = video_drvdata(file);
	int ret =0;
	ret = isp_dev_call(camdev->isp, s_tlb_base, tlb_base);
	return ret;
}
/**
		...
		struct v4l2_format *f)
{
	...
	if (f->fmt.pix.priv & V4L2_CAP_LIVE_MODE) {
		camdev->isp->preview_mode = V4L2_CAP_LIVE_MODE;
	} else {
		camdev->isp->preview_mode = V4L2_CAP_NORMAL_MODE;
	}

	if (f->fmt.pix.priv & V4L2_CAP_LOWFPS_MODE) {
		csd->priv = V4L2_CAP_LOWFPS_MODE;
	} else if (f->fmt.pix.priv & V4L2_CAP_QUITAK_MODE) {
		csd->priv = V4L2_CAP_QUITAK_MODE;
	} else {
		csd->priv = V4L2_CAP_NORMAL_MODE;
	}
	...
	if (ret < 0 && ret != -ENOIOCTLCMD) {
		ISP_PRINT(ISP_ERROR,"Failed to set isp format of s_fmt\n");
		return -EINVAL;
	}
...
static int ovisp_vidioc_enum_framesizes(struct file *file, void *priv,
				      struct v4l2_frmsizeenum *sizes)
{
	...
	struct ovisp_camera_subdev *csd = &camdev->csd[camdev->input];
	int ret = 0;
	ret = v4l2_subdev_call(csd->sd, video, enum_framesizes, sizes);
	if (ret < 0) {
		ISP_PRINT(ISP_WARNING,"Failed to get ovisp_vidioc_enum_framesizes \n");
		return -EINVAL;
	}
	return 0;
		...
		unsigned int index)
{
	...
	struct ovisp_fh *fh = priv;
	int ret;
	ISP_PRINT(ISP_INFO,"FUNC: %s, index = %d, camdev->input = %d\n", __func__, index, camdev->input);
	if (ovisp_camera_active(camdev))
		return -EBUSY;
...
static int ovisp_acquire_photo(struct ovisp_camera_dev *camdev, struct v4l2_control *ctrl)
{
	...
	parm = (struct v4l2_acquire_photo_parm *)(ctrl->value);
	...
	/* flush cache */
	ret = ovisp_flush_cache(camdev, parm->src.addr, parm->src.length, parm->src.memory, DMA_TO_DEVICE);
	if (ret) {
		ISP_PRINT(ISP_ERROR,"Failed to flush cache!\n");
		return -EFAULT;
	}
	ret = ovisp_flush_cache(camdev, parm->dst.addr, parm->dst.length, parm->dst.memory, DMA_FROM_DEVICE);
	if (ret) {
		ISP_PRINT(ISP_ERROR,"Failed to flush cache!\n");
		return -EFAULT;
	}
	...
	if (parm->flags & V4L2_ACQUIRE_DELAY_PHOTO) {
		ISP_PRINT(ISP_INFO,"%s==========%d\n", __func__, __LINE__);
		/*to match the sensor supported format*/
		cfmt.vmfmt.width = parm->src_width;
		cfmt.vmfmt.height = parm->src_height;
		cfmt.vmfmt.priv = csd->priv;
		memset(&cfmt.fmt_data, 0, sizeof(cfmt.fmt_data));
		...
		tmp_ifmt.vfmt.width = parm->src_width;
		tmp_ifmt.vfmt.height = parm->src_height;
		...
		/* get isp preview info (1f200 - ISP_OUTPUT_INFO_LENS) */
		...
		if(cfmt.vmfmt.width != save_ifmt.vfmt.dev_width
		   || cfmt.vmfmt.height != save_ifmt.vfmt.dev_height) {
			if(si->write_mode == GROUP_LUNCH){
				ISP_PRINT(ISP_INFO,"write_mode == GROUP_LUNCH\n");
				ret = v4l2_subdev_call(csd->sd, video, s_mbus_fmt, &cfmt.vmfmt);
				if (ret && ret != -ENOIOCTLCMD)
					return -EINVAL;
				ret = isp_dev_call(camdev->isp, get_camera_info, &cfmt.fmt_data);
				...
				if (si->write_mode == ISP_GROUP_WRITE) {
					...
					ret = isp_dev_call(camdev->isp, set_camera_fmt, &cfmt.vmfmt, &grp);
					if (ret && ret != -ENOIOCTLCMD) {
						ISP_PRINT(ISP_ERROR,"Failed to set device format(ISP_GROUP_WRITE) \n");
						return -EINVAL;
					}
				} else if (si->write_mode == I2C_WRITE) {
					/* I2C_WRITE mode */
					ret = v4l2_subdev_call(csd->sd, video, s_mbus_fmt, &cfmt.vmfmt);
					if (ret && ret != -ENOIOCTLCMD) {
						ISP_PRINT(ISP_ERROR,"Failed to set device format(I2C_WRITE) \n");
						return -EINVAL;
					}
				}
...
#if defined CAPTURE_INITAL_MODE
				/* phy reset */
				ret = isp_dev_call(camdev->isp, pre_fmt, &tmp_ifmt);
				if (ret < 0 && ret != -ENOIOCTLCMD) {
					ISP_PRINT(ISP_ERROR,"Failed to set isp format of pre_fmt\n");
					return -EINVAL;
				}
		...
		if (ret) {
			ISP_PRINT(ISP_WARNING, "%s[%d] bypass_capture failed!\n",__func__, __LINE__);
			ret = v4l2_subdev_call(csd->sd, core, soft_reset, 0x04);
			if (ret < 0) {
				ISP_PRINT(ISP_ERROR, "%s[%d] reset sensor failed!\n",__func__, __LINE__);
				return -EINVAL;
			} else {
				ret = isp_dev_call(camdev->isp, bypass_capture, &tmp_ifmt, parm->src.addr);
				if (ret < 0) {
					ISP_PRINT(ISP_ERROR, "%s[%d] bypass_capture failed again!\n",__func__, __LINE__);
					return -EINVAL;
				}
			}
		}
		...
		isp_dev_call(camdev->isp, get_exif_info, parm);

		tmp_ifmt.vfmt.dev_width = parm->src_width;
		tmp_ifmt.vfmt.dev_height = parm->src_height;
		tmp_ifmt.vfmt.dev_fourcc = parm->src.fourcc;
		tmp_ifmt.vfmt.width = parm->dst_width;
		tmp_ifmt.vfmt.height = parm->dst_height;
		tmp_ifmt.vfmt.fourcc = parm->dst.fourcc;
		tmp_ifmt.vfmt.field = camdev->frame.field;

		ret = isp_dev_call(camdev->isp, process_raw, &tmp_ifmt, parm->src.addr, parm->dst.addr, buf->priv, parm->zoomVal);
		if (ret) {
			ISP_PRINT(ISP_WARNING, "%s[%d] process_raw failed!\n",__func__, __LINE__);
			ret = v4l2_subdev_call(csd->sd, core, soft_reset, 0x06);
			if (ret < 0) {
				ISP_PRINT(ISP_ERROR, "%s[%d] reset sensor failed!\n",__func__, __LINE__);
				return -EINVAL;
			} else {
				ret = isp_dev_call(camdev->isp, process_raw, &tmp_ifmt, parm->src.addr, parm->dst.addr, buf->priv, parm->zoomVal);
				if (ret < 0) {
					ISP_PRINT(ISP_ERROR, "%s[%d] process_raw failed again!\n",__func__, __LINE__);
					return -EINVAL;
				}
			}
		}

		if(parm->flags & V4L2_ACQUIRE_QUICK_PHOTO)
			return ret;
		if(cfmt.vmfmt.width != save_ifmt.vfmt.dev_width
		   || cfmt.vmfmt.height != save_ifmt.vfmt.dev_height) {
			...
			if (si->write_mode == GROUP_LUNCH) {
				...
				/* phy reset */
				/* ret = isp_dev_call(camdev->isp, pre_fmt, &save_ifmt); */
				/* if (ret < 0 && ret != -ENOIOCTLCMD) { */
				/*     ISP_PRINT(ISP_ERROR,"Failed to set isp format of pre_fmt\n"); */
				/*     return -EINVAL; */
				/* } */
		...
		if (ret) {
			ISP_PRINT(ISP_WARNING, "%s[%d] restore_flags failed!\n", __func__, __LINE__);
			ret = v4l2_subdev_call(csd->sd, core, soft_reset, 0x02);
			if (ret < 0) {
				ISP_PRINT(ISP_ERROR, "%s[%d] reset sensor failed!\n",__func__, __LINE__);
				return -EINVAL;
				...
				ret = isp_dev_call(camdev->isp, restore_flags, &save_ifmt);
				if (ret < 0) {
					ISP_PRINT(ISP_ERROR, "%s[%d] restore_flags failed again!\n",__func__, __LINE__);
					return -EINVAL;
				}
			}
		}
...
static int ovisp_capture_raw(struct ovisp_camera_dev *camdev, struct v4l2_control *ctrl)
{
	return 0;
}

static int ovisp_process_raw(struct ovisp_camera_dev *camdev, struct v4l2_control *ctrl)
{
	/* struct ovisp_camera_subdev *csd = &camdev->csd[camdev->input]; */
	struct isp_format tmp_ifmt;
	struct isp_format save_ifmt;
	...
	ret = isp_dev_call(camdev->isp, process_raw, &tmp_ifmt, parm.src.addr, parm.dst.addr, buf->priv, parm.zoomVal);
	if (ret) {
		ISP_PRINT(ISP_ERROR,"%s[%d] process_raw failed!\n",__func__, __LINE__);
		return -EINVAL;
	}
	...
	ret = isp_dev_call(camdev->isp, restore_flags, &save_ifmt);
	if (ret) {
		ISP_PRINT(ISP_ERROR,"%s[%d] restore_flags failed!\n",__func__, __LINE__);
		return -EINVAL;
	}
		...
		struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_ACQUIRE_PHOTO:
		...
		break;
	case V4L2_CID_CAPTURE_RAW:
		ret = ovisp_capture_raw(camdev, ctrl);
		break;
	case V4L2_CID_PROCESS_RAW:
		ret = ovisp_process_raw(camdev, ctrl);
		break;
	case TURN_OVSENSOR_TESTMODE:
		ret = v4l2_subdev_call(csd->sd, core, s_ctrl_1, ctrl,TURN_OVSENSOR_TESTMODE);
		break;
	case TURN_OVSENSOR_NORMALMODE:
		ret = v4l2_subdev_call(csd->sd, core, s_ctrl_1, ctrl,TURN_OVSENSOR_NORMALMODE);
		break;
	case CHECK_IIC_CHIP_ID:
		ret = v4l2_subdev_call(csd->sd, core, s_ctrl_iic, ctrl);
		break;
	default:
		if (csd->bypass)
		...
		struct v4l2_control *ctrl)
{
	...
	ISP_PRINT(ISP_INFO,"%s==========%d\n", __func__, __LINE__);
	switch(ctrl->id) {
	case V4L2_CID_GET_AF_FLAG:
		ctrl->value = (bool)(csd->sensor_af_flag);
		break;
	default:
		if (csd->bypass)
...
#ifdef CONFIG_VIDEO_JZ_HS_ISP
	if(getCameraKey_WakeUp()){
	    ISP_PRINT(ISP_ERROR,"hstp start, you can not use isp\n");
	    return -1;
	}
#endif
...
static int ovisp_v4l2_close(struct file *file)
{
	...
	if (--camdev->refcnt == 0) {
		...
		ovisp_subdev_power_off(camdev, camdev->csd[camdev->input].client, camdev->input);
		isp_dev_call(camdev->isp, release, NULL);
		camdev->input = -1;
	}
	/* Close the priority */
...
static int ovisp_camera_probe(struct platform_device *pdev)
{
	...
	for (i = 0 ; i < pdata->client_num; i++) {
		ISP_PRINT(ISP_INFO,"sensor: %s\n", pdata->client[i].board_info->type);
		if (!(pdata->client[i].flags & CAMERA_CLIENT_POWER_SUPPLY_ALWAYS)) {
...
#if defined(CONFIG_VIDEO_OV5640) && defined(CONFIG_BOARD_MONDAY)
	pmu_dvdd = regulator_get(camdev->dev, "cpu_dvdd");
	if(IS_ERR(camdev->camera_power)) {
		dev_warn(camdev->dev, "camera regulator missing\n");
		ret = -ENXIO;
		goto regulator_error;
	...
	if (ret) {
		ISP_PRINT(ISP_ERROR,"Failed to register v4l2 device of subdev\n");
		ret = -ENOMEM;
		goto cleanup_ctx;
	}
	/* Initialize queue. */
