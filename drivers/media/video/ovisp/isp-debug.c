...
void dump_isp_timestamp(struct isp_device *isp)
{
	/*
	  Time stamp
	  BaseAddress:
	  0x1e000
	  unsigned int nInitTime;		// 0x768~0x76b
	  unsigned int nCMDStartTime;		// 0x76c~0x76f
	  unsigned int nCMDEndTime;		// 0x770~0x773
	  unsigned int nCommandAbortTime;	// 0x774~0x777

	  unsigned int nVsyncTime;      	// 0x778~0x77b
	  unsigned int nIDIEOFTime;		// 0x77c~0x77f
	  unsigned int nEOFTime;		// 0x780~0x783
	  unsigned int nEOF1TimeStamp;		// 0x784~0x787
	  unsigned int nEOF2TimeStamp;		// 0x788~0x78b
	  unsigned int nEOF3TimeStamp;		// 0x78c~0x78f
	  unsigned int nMAC1InterruptTime1;	// 0x790~0x793
	  unsigned int nMAC1InterruptTime2;	// 0x794~0x797
	*/
	int i = 0;
	printk("*********************************nInitTime***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e768 + i, isp_firmware_readb(isp, 0x1e768 + i));
	}
	printk("*********************************nCMDStartTime***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e76c + i, isp_firmware_readb(isp, 0x1e76c + i));
	}
	printk("*********************************nCMDEndTime***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e770 + i, isp_firmware_readb(isp, 0x1e770 + i));
	}
	printk("*********************************nCommandAbortTime***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e774 + i, isp_firmware_readb(isp, 0x1e774 + i));
	}
	printk("*********************************nVsyncTime***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e778 + i, isp_firmware_readb(isp, 0x1e778 + i));
	}
	for (i = 0; i < 8; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e77c + i, isp_firmware_readb(isp, 0x1e77c + i));
	}
	printk("*********************************EOF***************************\n");
	for (i = 0; i < 16; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e784 + i, isp_firmware_readb(isp, 0x1e784 + i));
	}
	printk("*****************************nMAC1InterruptTime1***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e790 + i, isp_firmware_readb(isp, 0x1e790 + i));
	}
	printk("*****************************nMAC1InterruptTime2***************************\n");
	for (i = 0; i < 4; i++) {
		printk("Reg [ 0x%04x ] = 0x%02x\n", 0x1e794 + i, isp_firmware_readb(isp, 0x1e794 + i));
	}
}
...
void dump_isp_range_regs(struct isp_device * isp, unsigned int addr, int count)
{
	...
	for(i = 0; i < count; i++)
		ISP_PRINT(ISP_INFO,"[0x%x]:	0x%x\n", addr + i,isp_reg_readb(isp, addr + i));
