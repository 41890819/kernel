...
static long efuse_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_READ_CHIP_ID:
		jz_efuse_id_read(1, chipid_buf);
		if (copy_to_user((void *)arg, chipid_buf, sizeof(chipid_buf))) {
			ret = -EFAULT;
		}
		break;
	default:
...
static int jz_efuse_probe(struct platform_device *pdev)
{
	...
	res = create_proc_entry("chip_id", 0444, p);
	if (res) {
		res->read_proc = efuse_read_chip_id_proc;
		res->write_proc = NULL;
		res->data = NULL;
	}
	res = create_proc_entry("user_id", 0444, p);
	if (res) {
...
void jz_efuse_id_read(int seg_id, uint32_t *buf)
{
	if (extra_efuse == NULL) {
