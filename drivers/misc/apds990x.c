...
static int apds990x_configure(struct apds990x_chip *chip)
{
	...
	apds990x_write_byte(chip, APDS990X_PPCOUNT, /*chip->pdata->ppcount*/0xF);
...
{
        int err	 = 0;/*regulator_bulk_enable(ARRAY_SIZE(chip->regs),
		     chip->regs);*/
	if (err < 0)
		return err;
				...
				const struct i2c_device_id *id)
{
	...
	chip->pdrive = /*chip->pdata->pdrive*/2;
