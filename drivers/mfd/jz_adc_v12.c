...
static inline void jz_adc_irq_set_masked(struct jz_adc *adc, int irq, bool masked)
{
	unsigned long flags;
	uint32_t val;
	...
	val = readl(adc->base + JZ_REG_ADC_CTRL);
	if (masked) {
		val |= BIT(irq);
	} else {
		val &= ~BIT(irq);
	}
	writel(val, adc->base + JZ_REG_ADC_CTRL);
...
static void jz_adc_irq_ack(struct irq_data *data)
{
	...
	writel(BIT(irq), adc->base + JZ_REG_ADC_STATUS);
}
...
static inline void jz_adc_enable(struct jz_adc *adc)
{
	...
	if (atomic_inc_return(&adc->clk_ref) == 1) {
		...
		val &= ~BIT(7);
		writel(val, adc->base + JZ_REG_ADC_ENABLE);
...
// Enable / Disable AUX/BAT
static inline void jz_adc_set_enabled(struct jz_adc *adc, int engine, bool enabled)
{
	unsigned long flags;
	uint32_t val;
...
// Export AUX Config
int jz_adc_set_config(struct device *dev, uint32_t cmd)
{
	struct jz_adc *adc = dev_get_drvdata(dev->parent);
	unsigned long flags;
	...
	spin_lock_irqsave(&adc->lock, flags);
	writel(cmd, adc->base + JZ_REG_ADC_CFG);
	spin_unlock_irqrestore(&adc->lock, flags);
...
static int __devinit jz_adc_probe(struct platform_device *pdev)
{
	...
	adc->mem = request_mem_region(mem_base->start, JZ_REG_ADC_STATUS, pdev->name);
	if (!adc->mem) {
		ret = -EBUSY;
	...
	for (irq = adc->irq_base; irq < adc->irq_base + SADC_NR_IRQS; ++irq) {
		...
		irq_set_chip_and_handler(irq, &jz_adc_irq_chip, handle_level_irq);
	}
	...
	clk_enable(adc->clk);
	writel(0x80, adc->base + JZ_REG_ADC_ENABLE);
	writel(0xff, adc->base + JZ_REG_ADC_CTRL);
	writel((CLKDIV - 1), adc->base + JZ_REG_ADC_CLKDIV);
