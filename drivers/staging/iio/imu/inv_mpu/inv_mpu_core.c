 ...
 *               MPU3050/MPU6050/MPU9150/MPU6500/MPU9250/MPU9255 devices.
 */
...
static int hardwaredet_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct mpu_platform_data * pdata = (struct mpu_platform_data *)dev_get_platdata(&client->dev);

	if (pdata->board_init) {
		ret = pdata->board_init(&client->dev);
		if (ret < 0) {
			printk("MPU board init fail\n");
			goto check_fail;
		}
	}
 ...
 check_fail:
	printk("Check MPU FAIL\n");
	return 0;
}

static struct i2c_driver mpu_detect_driver = {
	.class          =  I2C_CLASS_HWMON,
...
static int __init mpu_detect_init(void)
{
        return i2c_add_driver(&mpu_detect_driver);
}

hardwaredet_initcall(mpu_detect_init);
#endif
