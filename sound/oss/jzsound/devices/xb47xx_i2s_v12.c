...
extern int hstp_codec_ready(struct codec_info *codec_dev);
...
int i2s_replay_hstp(struct snd_dev_data *ddata)
{
	int ret = 0;
	int count = 0;
	int write_count = 0;
	int tfs_bit = 0;
	int total = sizeof(hstp_sound_data)/2;
	int gol_val,gor_val,godl_val,godr_val;
	struct i2s_device *i2s_dev = i2s_get_private_data(ddata);
	struct codec_info * cur_codec = i2s_dev->cur_codec;
	/* struct dsp_pipe *dp = NULL; */
	/* struct dsp_endpoints *endpoints = NULL; */
	spinlock_t hstp_lock;

	spin_lock_init(&hstp_lock);
	/* if (ddata == NULL) */
	/* 	return -ENODEV; */
	/* if(ddata->priv_data) { */
	/* 	endpoints = ddata->get_endpoints(ddata); */
	/* 	if (endpoints == NULL) */
	/* 		return -ENODEV; */
	/* }else{ */
	/* 	return -ENODEV; */
	...
	gol_val = __codec_get_gol(cur_codec);
	gor_val = __codec_get_gor(cur_codec);
	godl_val = __codec_get_godl(cur_codec);
	godr_val = __codec_get_godr(cur_codec);

	ret = hstp_codec_ready(cur_codec);
	if(ret < 0)
		return -1;
	
	__codec_set_gol(cur_codec, 10);
	__codec_set_gor(cur_codec, 10);
	__codec_set_godl(cur_codec, 6);
	__codec_set_godr(cur_codec, 6);

	__i2s_set_transmit_trigger(i2s_dev, 15);
	//mutex_lock(&dp->mutex);	
	spin_lock(&hstp_lock);
	__i2s_enable_byteswap(i2s_dev);
	for(count = 0; count < total/30; count++){
		for(write_count = count*30; write_count < (count + 1)*30; write_count++){
			__i2s_write_tfifo(i2s_dev, hstp_sound_data[write_count]);
		}
		if(count == 0){
		...
		do{
			i++;
			tfs_bit = __i2s_test_tfs(i2s_dev);
			if(i > 100000){
				dump_i2s_reg(i2s_dev);
				WARN_ON(1);
			}
	...
	spin_unlock(&hstp_lock);

	//mutex_unlock(&dp->mutex);
	__codec_set_gol(cur_codec, gol_val);
	__codec_set_gor(cur_codec, gor_val);
	__codec_set_godl(cur_codec, godl_val);
	__codec_set_godr(cur_codec, godr_val);

	return 0;

}
EXPORT_SYMBOL(i2s_replay_hstp);
...
static long do_ioctl_work(struct i2s_device *i2s_dev, unsigned int cmd, unsigned long arg)
{
	...
	case SND_DSP_SET_REPLAY_VOL:
		...
		break;
	case SND_GLS_SET_REPLAY_VOL:
		if (cur_codec)
			ret = cur_codec->codec_ctl_2(cur_codec, CODEC_GLS_SET_REPLAY_VOLUME, arg);
		break;
	case SND_GLS_SET_DFT_VOL:
		if (cur_codec)
			ret = cur_codec->codec_ctl_2(cur_codec, CODEC_GLS_SET_DFT_VOLUME, arg);
		break;
		...
		int hook_active_level)
{
	...
	if (type == SND_SWITCH_TYPE_GPIO && hp_det != NULL) {
		...
		switch_data.usb_valid_level = usb_det->active_level;
	} else {
	...
	if (type == SND_SWITCH_TYPE_GPIO && usb_det != NULL) {
		switch_data.usb_gpio = usb_det->gpio;
	} else {
		switch_data.usb_gpio = -1;
	}
