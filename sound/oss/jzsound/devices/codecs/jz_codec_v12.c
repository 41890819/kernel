	...
	st_ns = sched_clock();
        for (i=0; i<3000; i++) {
                ...
                if (__read_codec_reg(codec_dev, reg) & (1 << bit))
                        return;
		if(sched_clock() - st_ns > 1000 * 1000*1000LL)
			break;
        }
...
static void codec_print_ioc_cmd(int cmd)
{
	...
	int cmd_arr[] = {
		...
		CODEC_DUMP_GPIO,                CODEC_GLS_SET_REPLAY_VOLUME,
		CODEC_GLS_SET_DFT_VOLUME
	};
...
static void dump_gpio_state(struct codec_info *codec_dev)
{
	...
	if(codec_platform_data &&
	   (codec_platform_data->gpio_usb_detect.gpio != -1)) {
		val = __gpio_get_value(codec_platform_data->gpio_usb_detect.gpio);
		printk("gpio usb detect %d statue is %d.\n",codec_platform_data->gpio_usb_detect.gpio, val);
	}

	if(codec_platform_data &&
	   (codec_platform_data->gpio_usb_sel.gpio != -1)) {
		val = __gpio_get_value(codec_platform_data->gpio_usb_sel.gpio);
		printk("gpio usb sel %d statue is %d.\n",codec_platform_data->gpio_usb_sel.gpio, val);
	}
}
...
static void codec_set_hp(struct codec_info *codec_dev, int mode)
{
	...
	case HP_ENABLE:
		if (__codec_get_sb_hp(codec_dev) == POWER_OFF) {
			...
			/*set route to reduce pop */
			if (codec_platform_data->gpio_usb_sel.gpio != -1 ){
				gpio_direction_output(codec_platform_data->gpio_usb_sel.gpio,
						      !codec_platform_data->gpio_usb_sel.active_level);
			}	
#endif
...
static int codec_gls_replay_volume(struct codec_info *codec_dev, int *val);
...
extern int ampvdd_power_init(struct device *dev);
extern int ampvdd_power_on(void);
static void gpio_enable_spk_en(struct codec_info *codec_dev)
{
	...
	if(codec_platform_data && (codec_platform_data->gpio_spk_en.gpio != -1)) {
		...
		ampvdd_power_init(dev);	
		ampvdd_power_on();
		msleep(50);  /*Wait for the power amplifier response time*/
	}
}

extern int ampvdd_power_off(void);
...
static int codec_init(struct codec_info *codec_dev)
{
	...
	__codec_switch_sb_micbias2(codec_dev, POWER_ON);
      	...
      	usb_audio_select();//set route to reduce pop
...
static int codec_suspend(struct codec_info *codec_dev)
{
	...
	save_spk_state = spk_enable;

	g_codec_sleep_mode = 0;

#ifdef CONFIG_BOARD_PARAMOUNT
	/*reduce pop*/
	val_hpdet = gpio_get_value(codec_platform_data->gpio_hp_detect.gpio);
	val_usbdet = gpio_get_value(codec_platform_data->gpio_usb_detect.gpio);
	if((val_hpdet == 0) && (val_usbdet == 1)){
		if (codec_platform_data->gpio_usb_sel.gpio != -1 ){
			gpio_direction_output(codec_platform_data->gpio_usb_sel.gpio,
					      !codec_platform_data->gpio_usb_sel.active_level);
		}
...
#define BOARD_REPLAY_VOLUME 0
// default is 0
// if you want reduce volume, please add the BOARD_REPLAY_VOLUME from 0 to 15
static int codec_gls_set_replay_volume(struct codec_info *codec_dev, int *val){
        if (*val < 0 || *val > 31) {
	        return -EINVAL;
	}

	*val += BOARD_REPLAY_VOLUME;

	printk("codec_gls_set_replay_volume in %d\n", *val);
	__codec_set_gol(codec_dev, *val+4);
	__codec_set_gor(codec_dev, *val+4);
	__codec_set_godl(codec_dev, *val);
	__codec_set_godr(codec_dev, *val);
...
#define GLS_DFT_HP_VOL 8
#define GLS_DFT_DAC_VOL 2
static int codec_gls_set_dft_volume(struct codec_info *codec_dev, int *val){
	int volume_base = 0;
	int fixed_vol;
	...
	*val += BOARD_REPLAY_VOLUME;
	...
	//printk("codec_gls_set_dft_volume:%d\n", fixed_vol);
	/* __codec_set_gol(codec_dev, fixed_vol); */
	/* __codec_set_gor(codec_dev, fixed_vol); */
	/* __codec_set_godl(codec_dev, fixed_vol); */
	/* __codec_set_godr(codec_dev, fixed_vol); */

	*val += BOARD_REPLAY_VOLUME;
	printk("codec_gls_set_dft_volume:%d\n", *val);//hpwang set default volume hp:8 dac:2
	__codec_set_gol(codec_dev, GLS_DFT_HP_VOL);
	__codec_set_gor(codec_dev, GLS_DFT_HP_VOL);
	__codec_set_godl(codec_dev, GLS_DFT_DAC_VOL);
	__codec_set_godr(codec_dev, GLS_DFT_DAC_VOL);
...
static int hstp_get_hp_state(struct codec_info *codec_dev)
{
	int usbdete_state = 0;
	int hpdete_state = 0;
	int hp_state = 0;
	if(codec_platform_data && (codec_platform_data->gpio_hp_detect.gpio != -1) &&
	   (codec_platform_data->gpio_usb_detect.gpio != -1)) {
		usbdete_state  = __gpio_get_value(codec_platform_data->gpio_usb_detect.gpio);
		hpdete_state  = __gpio_get_value(codec_platform_data->gpio_hp_detect.gpio);
		if((usbdete_state == 1) && (hpdete_state == 1)){
			hp_state = 1;
		}else{
			hp_state = 0;
		}
		return hp_state;
	}else if(codec_platform_data && (codec_platform_data->gpio_hp_detect.gpio != -1) &&
		 (codec_platform_data->gpio_usb_detect.gpio == -1)){
		hpdete_state  = __gpio_get_value(codec_platform_data->gpio_hp_detect.gpio);
		return hpdete_state;
	}else
...
int hstp_codec_ready(struct codec_info *codec_dev)
{
	int hstp_hp_state = 0;

	codec_mute(codec_dev, 0, CODEC_WMODE);

	hstp_hp_state = hstp_get_hp_state(codec_dev);
	if(hstp_hp_state == 1){
		codec_set_device(codec_dev,5);
	}else if(hstp_hp_state == 0){
		codec_set_device(codec_dev,4);
	}else{
		printk("hstp codec ready error\n");
		return -1;
	}
...
static int jzcodec_ctl(struct codec_info *codec_dev, unsigned int cmd, unsigned long arg)
{
	...
	DUMP_IOC_CMD("enter");
	{
		...
		case CODEC_GLS_SET_REPLAY_VOLUME:
		        ret = codec_gls_set_replay_volume(codec_dev, (int*)arg);
			break;

		case CODEC_GLS_SET_DFT_VOLUME:
		        ret = codec_gls_set_dft_volume(codec_dev, (int*)arg);
			break;
...
static int jz_codec_probe(struct platform_device *pdev)
{
	...
	jz_set_hp_detect_type(SND_SWITCH_TYPE_GPIO,
			      ...
			      &codec_platform_data->gpio_mic_detect_en,
			      &codec_platform_data->gpio_buildin_mic_select,
			      &codec_platform_data->gpio_usb_detect,
			      codec_platform_data->hook_active_level);
#endif
	...
	if (codec_platform_data->gpio_usb_sel.gpio != -1 )
		if (gpio_request(codec_platform_data->gpio_usb_sel.gpio,"gpio_usb_sel") < 0) {
			gpio_free(codec_platform_data->gpio_usb_sel.gpio);
			gpio_request(codec_platform_data->gpio_usb_sel.gpio,"gpio_usb_sel");
		}
