						...
						struct snd_dev_data *ddata)
{
	...
	case SOUND_MIXER_DFT_GLASS:{
	        int vol = 0;
		printk("SOUND_MIXER_DFT_GLASS\n");
		if (ddata->dev_ioctl) {
			ret = (int)ddata->dev_ioctl(SND_GLS_SET_DFT_VOL, (unsigned long)&vol);
		} else if(ddata->dev_ioctl_2) {
			ret = (int)ddata->dev_ioctl_2(ddata, SND_GLS_SET_DFT_VOL, (unsigned long)&vol);
		}
		break;
	}

	case SOUND_MIXER_WRITE_GLASS:{
	  int vol = 0;
	  printk("SOUND_MIXER_WRITE_GLASS in\n");
	  if (get_user(vol, (int*)arg)){
	    return -EFAULT;
	  }
	  vol = vol & 0xff;
	  if (ddata->dev_ioctl) {
	    ret = (int)ddata->dev_ioctl(SND_GLS_SET_REPLAY_VOL, (unsigned long)&vol);
	  } else if(ddata->dev_ioctl_2) {
	    ret = (int)ddata->dev_ioctl_2(ddata, SND_GLS_SET_REPLAY_VOL, (unsigned long)&vol);
	  }
	  if (ret < 0)
	    return ret;
