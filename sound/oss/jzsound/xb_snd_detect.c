...
static void snd_switch_work(struct work_struct *hp_work)
{
	...
	if (switch_data->type == SND_SWITCH_TYPE_GPIO) {
		/*usb headphone*/
		if (switch_data->usb_gpio > 0){
			if(gpio_get_value(switch_data->usb_gpio) == (int)switch_data->usb_valid_level){		 
				state = 0;
		 	}else{
...
static void snd_switch_work_resume(struct work_struct *hp_work)
{
	int state = 0;
	int tmp_state =0;
	int i = 0;
...
static int snd_switch_resume(struct platform_device *pdev)
{
	...
	snd_switch_work_resume(&switch_data->hp_work); // we will enable_irq(switch_data->hp_irq) in this work;
