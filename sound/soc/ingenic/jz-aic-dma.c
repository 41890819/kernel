/* sound/soc/ingenic/jz-aic-dma.c
 * AIC DMA driver for Ingenic SoC.
 *
 * Copyright (C) 2014 Ingenic Semiconductor Co., Ltd.
 *	http://www.ingenic.com
 * Author:
 *	V1.0 : Lutts Wolf <slcao@ingenic.cn>
 *	V1.1 : Sun Jiwei <jwsun@ingenic.cn>
 *
...
#include "include/jz-pdma.h"

static struct device *parent;
static u64 jz_pcm_dmamask = DMA_BIT_MASK(32);
...
struct jz_dma_buf_aic {
	struct jz_dma_buf_aic	*next;
...
static bool jz_pcm_dma_filter(struct dma_chan *chan, void *filter_param)
{
		...
		void *param )
{
	...
	if(dev_id == SNDRV_PCM_STREAM_PLAYBACK){
		replay_channel = dma_request_channel(mask,
				jz_pcm_dma_filter, NULL);
	...
	if(dev_id == SNDRV_PCM_STREAM_CAPTURE){
		record_channel = dma_request_channel(mask,
				jz_pcm_dma_filter, NULL);
...
struct jz_runtime_data {
	...
	dma_addr_t dma_pos;
	dma_addr_t dma_end;
	struct jz_pcm_dma_params *params;
	...
	struct jz_dma_buf_aic	*curr;		/* current dma buffer */
	struct jz_dma_buf_aic	*next;		/* next buffer to load */
	struct jz_dma_buf_aic	*end;		/* end of queue */
 ...
 *
*/
static int jz_dma_buf_enqueue(struct jz_runtime_data *prtd, dma_addr_t data, int size)
{
	struct jz_dma_buf_aic *aic_buf;

	aic_buf = kzalloc(sizeof(struct jz_dma_buf_aic), GFP_ATOMIC);  //change it from GFP_KERNEL
	if (aic_buf == NULL) {
...
static unsigned int fifo_addr = 0;
void jz_pcm_start_normal_dma(struct jz_runtime_data *prtd,
			     int chan, unsigned long phyaddr, int count, int mode)
{
	...
	start_time = jiffies;
	while (REG_DMAC_DCCSR(chan) & (DMAC_DCCSR_HLT |
	...
	if (DMA_MODE_WRITE == mode) {
		...
		dst_addr = CPHYSADDR(fifo_addr);
		req_src = DMAC_DRSR_RS_AICOUT;
	} else {
		dma_cmd |= DMAC_DCMD_DAI;
		src_addr = CPHYSADDR(fifo_addr);
...
void audio_start_dma(struct jz_runtime_data *prtd, int mode)
{
	struct jz_dma_buf_aic *aic_buf;
	int channel;
...
*/
static void jz_pcm_enqueue(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct jz_runtime_data *prtd = runtime->private_data;
	...
	while (prtd->dma_loaded < prtd->dma_limit) {
		...
		ret = jz_dma_buf_enqueue(prtd, pos, len);
		if (ret == 0) {
			prtd->dma_loaded++;
...
static void schedule_next_period(struct snd_pcm_substream *substream) {
	...
	local_irq_restore(flags);
        ...
        spin_lock(&prtd->lock);
        jz_pcm_enqueue(substream);
	spin_unlock(&prtd->lock);
}

/*
 * call the function:jz_pcm_dma_callback() after DMA has transfered the current buffer
 */
void  jz_pcm_dma_callback(void *param)
{
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (!replay_dma_inited) {
			ret = jz_request_dma(SNDRV_PCM_STREAM_PLAYBACK,
					jz_pcm_dma_callback, (void *)substream);
			if (ret < 0) {
		...
		if (!record_dma_inited) {
			ret = jz_request_dma(SNDRV_PCM_STREAM_CAPTURE,
					jz_pcm_dma_callback, (void *)substream);
			if (ret < 0) {
...
static int jz_pcm_hw_free(struct snd_pcm_substream *substream)
{
	struct jz_runtime_data *prtd = substream->runtime->private_data;
...
static int jz_dma_ctrl(int channel)
{
	disable_dma(channel);
...
static int jz_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	jz_dma_ctrl(prtd->params->channel);
...
static void jz_pcm_clean(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct jz_runtime_data *prtd = runtime->private_data;
        struct jz_dma_buf_aic *aic_buf = NULL;
...
static int jz_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		break;
	default:
		printk("%s,Don't support this cmd(0x%x)\n", __func__, cmd);
		ret = -EINVAL;
	}
...
jz_pcm_pointer(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct jz_runtime_data *prtd = runtime->private_data;
	struct jz_dma_buf_aic *aic_buf = prtd->curr;
...
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct jz_runtime_data *prtd;

	PCM_DEBUG_MSG("enter jz_pcm_open, substream is %s\n",
		  (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ?
		  "playback" : "capture");

        snd_soc_set_runtime_hwparams(substream, &jz_pcm_hardware);
	prtd = kzalloc(sizeof(struct jz_runtime_data), GFP_KERNEL);
	if (prtd == NULL) {
			printk("not support capture %s %d\n", __func__, __LINE__);
		return -ENOMEM;
	}
...
static int jz_pcm_close(struct snd_pcm_substream *substream)
{
	...
	PCM_DEBUG_MSG("enter jz_pcm_close, substream is %s\n",
		      (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ?
		      "playback" : "capture");
			   ...
			   struct vm_area_struct *vma)//include/linux/mm.h
{
	...
	if ((vma->vm_end - vma->vm_start + off) > len) {
		pr_err("DMA error,vm_end:%ld,vm_start:%ld\n", vma->vm_end,
				vma->vm_start);
		return -EINVAL;
	}
...
static int jz_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	struct snd_pcm_substream *substream = pcm->streams[stream].substream;
	struct snd_dma_buffer *buf = &substream->dma_buffer;
	size_t size = jz_pcm_hardware.buffer_bytes_max;
...
static void jz_pcm_free_dma_buffers(struct snd_pcm *pcm)
{
...
int jz_pcm_new(struct snd_card *card, struct snd_soc_dai *dai,
	struct snd_pcm *pcm)
{
	int ret = 0;

	if (!card->dev->dma_mask)
		card->dev->dma_mask = &jz_pcm_dmamask;
	if (!card->dev->coherent_dma_mask)
		card->dev->coherent_dma_mask = DMA_BIT_MASK(32);

	if (dai->driver->playback.channels_min) {
		ret = jz_pcm_preallocate_dma_buffer(pcm,
...
static struct snd_soc_platform_driver jz__platform = {
	.ops            = &jz_pcm_ops,
	.pcm_new        = jz_pcm_new,
	.pcm_free       = jz_pcm_free_dma_buffers,
};

static int __devinit jz_platform_probe(struct platform_device *pdev)
{
	parent = pdev->dev.parent;
	return snd_soc_register_platform(&pdev->dev, &jz__platform);
}

static int __devexit jz_platform_remove(struct platform_device *pdev)
{
	jz_dma_release();
...
static struct platform_driver jz_dma_driver = {
	...
	.probe = jz_platform_probe,
	.remove = __devexit_p(jz_platform_remove),
};

static int __init jz_soc_platform_init(void)
{
	return platform_driver_register(&jz_dma_driver);
}
module_init(jz_soc_platform_init);

static void __exit jz_soc_platform_exit(void)
{
	platform_driver_unregister(&jz_dma_driver);
}
module_exit(jz_soc_platform_exit);

MODULE_AUTHOR("Sun Jiwei<jiwei.sun@ingenic.com>");
MODULE_VERSION("1.1");
MODULE_DESCRIPTION("Ingenic SoC Audio DMA module");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:jz-pcm-audio");
