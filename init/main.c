...
extern initcall_t __early_initcall_end[], __hardwaredet_initcall_start[],  __hardwaredet_initcall_end[];
static void __init do_initcalls_hardware(void)
{
	initcall_t *fn;

	for (fn = __early_initcall_end; fn < __hardwaredet_initcall_start; fn++)
		do_one_initcall(*fn);

	printk("\n\nKernel hardware detect start\n\n");

	for (fn = __hardwaredet_initcall_start; fn < __hardwaredet_initcall_end; fn++)
		do_one_initcall(*fn);
}
