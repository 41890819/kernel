...
static struct ricoh619_battery_platform_data ricoh619_battery_data = {
	...
	.type[0] = {
		.ch_vfchg 	= CH_VFCHG ,	  
		.ch_vrchg 	= CH_VRCHG,	  
		.ch_vbatovset 	= CH_VBATOVSET,	  
		.ch_ichg 	= CH_ICHG,	  
		.ch_ilim_adp 	= CH_ILIM_ADP,	  
		.ch_ilim_usb 	= CH_ILIM_USB,	  
		.ch_icchg 	= CH_ICCHG,	  
		.fg_target_vsys = FG_TARGET_VSYS,
		.fg_target_ibat = FG_TARGET_IBAT,
		.fg_poff_vbat 	= FG_POFF_VBAT,
		.fg_rsense_val	= FG_RSENSE_VAL,
		.jt_en 		= JT_EN,
		.jt_hw_sw 	= JT_HW_SW,
		.jt_temp_h 	= JT_TEMP_H,
		.jt_temp_l 	= JT_TEMP_L,
		.jt_vfchg_h 	= JT_VFCHG_H,
		.jt_vfchg_l 	= JT_VFCHG_L,
		.jt_ichg_h 	= JT_ICHG_H,
		.jt_ichg_l 	= JT_ICHG_L,
	},
	/*the battery of 4000mA 2000mA*/
	/*
	.type[1] = {
		.ch_vfchg 	= 0x02,
		.ch_vrchg 	= 0x4,
