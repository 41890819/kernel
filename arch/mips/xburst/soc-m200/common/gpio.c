...
#define GPIO_BASE_CAMERA_EN  0xb0010000
#define GPIO_GROUP_CAMERA_EN  0
#define GPIO_OFFSET_CAMERA_EN  (1 << 13)
#define GPIO_BASE_AF_EN  0xb0010300
#define GPIO_GROUP_AF_EN  3
#define GPIO_OFFSET_AF_EN  (1 << 28)
#endif
#if defined CONFIG_BOARD_CRUISE
#define GPIO_BASE_CAMERA_EN    0xb0010000
#define GPIO_GROUP_CAMERA_EN   0
#define GPIO_OFFSET_CAMERA_EN  (1 << 13)
#endif
#if defined CONFIG_BOARD_WESEEING
#define GPIO_BASE_CAMERA_EN    0xb0010000
#define GPIO_GROUP_CAMERA_EN   0
#define GPIO_OFFSET_CAMERA_EN  (1 << 13)
#endif
...
#define GPIO_BASE_CAMERA_EN    0xb0010300
#define GPIO_GROUP_CAMERA_EN   3
#define GPIO_OFFSET_CAMERA_EN  (1 << 4)
#endif
#endif//jz_hs_tp
...
void gpio_resume(void)
{
	int i;
	struct jzgpio_chip *jz;
	/* add for hstp ,keep camera PWDN and AF_EN status */
#ifdef CONFIG_VIDEO_JZ_HS_ISP
	int tmp[5];
	tmp[0] = 0;
#ifdef GPIO_BASE_CAMERA_EN
	tmp[0] = GPIO_OFFSET_CAMERA_EN & *(volatile int*)(GPIO_BASE_CAMERA_EN + 0x10);
	tmp[1] = GPIO_OFFSET_CAMERA_EN & *(volatile int*)(GPIO_BASE_CAMERA_EN + 0x20);
	tmp[2] = GPIO_OFFSET_CAMERA_EN & *(volatile int*)(GPIO_BASE_CAMERA_EN + 0x30);
	tmp[3] = GPIO_OFFSET_CAMERA_EN & *(volatile int*)(GPIO_BASE_CAMERA_EN + 0x40);
	tmp[4] = GPIO_OFFSET_CAMERA_EN & *(volatile int*)(GPIO_BASE_CAMERA_EN + 0x70);
	jz = &jz_gpio_chips[GPIO_GROUP_CAMERA_EN];
	jz->save[0] &= ~GPIO_OFFSET_CAMERA_EN;
	jz->save[0] |= tmp[0];
	jz->save[1] &= ~GPIO_OFFSET_CAMERA_EN;
	jz->save[1] |= tmp[1];
	jz->save[2] &= ~GPIO_OFFSET_CAMERA_EN;
	jz->save[2] |= tmp[2];
	jz->save[3] &= ~GPIO_OFFSET_CAMERA_EN;
	jz->save[3] |= tmp[3];
	jz->save[4] &= ~GPIO_OFFSET_CAMERA_EN;
	jz->save[4] |= tmp[4];
#endif

#ifdef GPIO_BASE_AF_EN
	tmp[0] = GPIO_OFFSET_AF_EN & *(volatile int*)(GPIO_BASE_AF_EN + 0x10);
	tmp[1] = GPIO_OFFSET_AF_EN & *(volatile int*)(GPIO_BASE_AF_EN + 0x20);
	tmp[2] = GPIO_OFFSET_AF_EN & *(volatile int*)(GPIO_BASE_AF_EN + 0x30);
	tmp[3] = GPIO_OFFSET_AF_EN & *(volatile int*)(GPIO_BASE_AF_EN + 0x40);
	tmp[4] = GPIO_OFFSET_AF_EN & *(volatile int*)(GPIO_BASE_AF_EN + 0x70);
	jz = &jz_gpio_chips[GPIO_GROUP_AF_EN];
	jz->save[0] &= ~GPIO_OFFSET_AF_EN;
	jz->save[0] |= tmp[0];
	jz->save[1] &= ~GPIO_OFFSET_AF_EN;
	jz->save[1] |= tmp[1];
	jz->save[2] &= ~GPIO_OFFSET_AF_EN;
	jz->save[2] |= tmp[2];
	jz->save[3] &= ~GPIO_OFFSET_AF_EN;
	jz->save[3] |= tmp[3];
	jz->save[4] &= ~GPIO_OFFSET_AF_EN;
	jz->save[4] |= tmp[4];
#endif
